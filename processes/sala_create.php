...
if (!isset($_SESSION['rol']) || !in_array($_SESSION['rol'], ['admin', 'manteniment'])) {
    header('Location: ../pages/admin_salas.php?error=NoPermisos');
    exit;
}

if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: ../pages/admin_salas.php');
    exit;
}
...
if ($nombre === '') {
    header('Location: ../pages/admin_salas.php?error=NombreRequerido');
    exit;
}
...
try {
    ...
    if (isset($_FILES['fondo']) && $_FILES['fondo']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($_FILES['fondo']['type'], $allowed)) {
            // Corregido: ruta relativa desde processes/ a img/regiones/
    ...
    header('Location: ../pages/admin_salas.php?ok=1');
    exit;
} catch (PDOException $e) {
    header('Location: ../pages/admin_salas.php?error=' . urlencode($e->getMessage()));
    exit;
}
